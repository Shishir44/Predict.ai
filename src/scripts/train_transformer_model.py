...
class TransformerTrainer:
    ...
    def train_model(self,
                  ...
                  y_soh_val: Optional[np.ndarray] = None,
                  y_soc_val: Optional[np.ndarray] = None,
                  epochs: int = 50,
                  batch_size: int = 32,
        ...
        if X_val is not None and y_soh_val is not None and y_soc_val is not None:
        ...
        # Save model if no validation set is used for early stopping
        if X_val is None:
            torch.save(self.model.state_dict(), self.model_dir / 'transformer_model.pth')
