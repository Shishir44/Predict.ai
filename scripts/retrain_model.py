...
class ModelRetrainer:
    def __init__(self, 
                ...
                retrain_thresholds: Dict[str, float] = None):
    ...
    def evaluate_new_model(self, 
        ...
        # Calculate metrics
        metrics = {
            'soh_mse': np.mean((predictions[0].flatten() - y_soh_test) ** 2),
            'soh_mae': np.mean(np.abs(predictions[0].flatten() - y_soh_test)),
            'soc_mse': np.mean((predictions[1].flatten() - y_soc_test) ** 2),
            'soc_mae': np.mean(np.abs(predictions[1].flatten() - y_soc_test))
        }
    ...
    def retrain(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.check_retrain_conditions(metrics):
