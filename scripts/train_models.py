...
class BatteryModelTrainer:
    ...
    def load_and_preprocess_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Load data
        data_path = self.data_dir / 'battery_data.csv'
        ...
        for i in range(len(X) - seq_length):
            ...
            y_soh.append(df['SOH'].iloc[i + seq_length])
            y_soc.append(df['SOC'].iloc[i + seq_length])
    ...
    def build_lstm_model(self) -> tf.keras.Model:
        ...
        model = models.Sequential([
            layers.LSTM(
                ...
                input_shape=(self.config['lstm']['sequence_length'], len(features))
            ),
            layers.Dropout(self.config['lstm']['dropout_rate']),
            layers.LSTM(self.config['lstm']['hidden_units']),
            layers.Dropout(self.config['lstm']['dropout_rate']),
            layers.Dense(2)  # Predict both SOH and SOC
        ])
